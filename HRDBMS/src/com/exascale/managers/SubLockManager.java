...
public class SubLockManager
{
	...
	{
		lock.lock();
		HashSet<Block> xlocks = xTXsToBlocks.get(tx.number());
		
		if (xlocks != null)
		{
			for (Block block : (HashSet<Block>)xlocks.clone())
			{
				if (!index.changedBlocks.contains(block))
				{
					unlockXLock(block, tx.number());
					index.myPages.remove(block);
				}
			}
		}
	...
	public void unlockXLock(Block b, long txnum)
	{
		lock.lock();
