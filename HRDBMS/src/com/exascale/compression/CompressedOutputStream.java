 ...
 * for communication link streams like tcp-sockets. The standard classes are
 * only file-capable (relying on EOF-detection). In comlinks, however, there are
 * several packets instead of a single file. Hence, the stream must be blocked,
 * i.e. the (varying) blocksize must become part of the protocol to allow the
 * corresponding input stream to detect the end of the packets. Furthermore, we
 * optimize to compress only packets larger than a certain size. All smaller
 * packets will be transferred uncompressed. For the packet size we use the
 * first two bytes in the packet (short), while MSBit signals whether the packet
 * is compressed or not. Because of this, the maximum buffer size is limited to
 * 32K - 1.
 */
public final class CompressedOutputStream extends FilterOutputStream
{
	...
	/**
	 * The maximum compression buffer size. The larger the buffer, the better
	 * the compression.
	 */
	 ...
	 * @param bufSize
	 *            the buffer size for compression. Packets larger than bufSize
	 *            are split.
	 * @param minCompressSize
	 *            the minimum compressed packet size. Smaller packets pass the
	 *            stream uncompressed.
	 */
	...
	/**
	 * Flushes the buffer. If the packet size is large enough, the data will be
	 * compressed. If the compressed data is smaller than the original data, a
	 * compressed packet will be written. Otherwise, an uncompressed packet is
	...
	private void flushBuffer() throws IOException
	{
		if (orgLen > 0)
		{
			...
			if (false)
			{
				...
				{
					final int num = deflater.deflate(defBuf, defLen, bufSize - defLen);
					if (num <= 0)
					{
						// can this really happen? Yes: if minCompressSize is
						// too small!
	 ...
	 * @param compressed
	 *            is true if packet is compressed, false if uncompressed
	 */
