...
public class HRDBMSStatement implements Statement
{
	...
	public boolean execute(String sql) throws SQLException
	{
		if (sql.toUpperCase().startsWith("SELECT") || sql.toUpperCase().startsWith("WITH"))
		{
			try
			{
				...
				sql = setParms(sql);
				byte[] command = "EXECUTEQ        ".getBytes(StandardCharsets.UTF_8);
				byte[] statement = sql.getBytes(StandardCharsets.UTF_8);
		...
		else
		{
			try
			{
				...
				sql = setParms(sql);
				byte[] command = "EXECUTEU        ".getBytes(StandardCharsets.UTF_8);
				byte[] statement = sql.getBytes(StandardCharsets.UTF_8);
				byte[] length = intToBytes(statement.length);
				conn.out.write(command);
				conn.out.write(length);
	...
	public Object get(String name, Object key) throws SQLException
	{
		try
		{
			byte[] command = "GET             ".getBytes(StandardCharsets.UTF_8);
	...
	public void put(String name, Object key, Object value) throws SQLException
	{
		try
		{
			byte[] command = "PUT             ".getBytes(StandardCharsets.UTF_8);
			byte[] from = stringToBytes(name);
			conn.out.write(command);
			conn.out.write(from);
			ObjectOutputStream objOut = new ObjectOutputStream(conn.out);
	...
	public void remove(String name, Object key) throws SQLException
	{
		try
		{
			byte[] command = "REMOVE          ".getBytes(StandardCharsets.UTF_8);
			byte[] from = stringToBytes(name);
			conn.out.write(command);
			conn.out.write(from);
			ObjectOutputStream objOut = new ObjectOutputStream(conn.out);
