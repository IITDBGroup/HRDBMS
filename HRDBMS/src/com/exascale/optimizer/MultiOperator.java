...
public final class MultiOperator implements Operator, Serializable
{
	...
	public void start() throws Exception
	{
		...
		{
			//double percentInMem = ResourceManager.QUEUE_SIZE * Double.parseDouble(HRDBMSWorker.getHParms().getProperty("hash_external_factor")) / childCard;
	...
	private void external(double percentInMem)
	{
		try
		{
			...
			final int numPar = Math.min(Runtime.getRuntime().availableProcessors(), 1800000000 / childCard);
			while (z < limit)
			{
				if (epThreads.size() >= numPar)
				{
					int k = numPar - 1;
					while (k >= 0)
					{
				...
				ExternalProcessThread thread = new ExternalProcessThread(lbins.get(z++));
				thread.start();
	...
	private class ExternalProcessThread extends HRDBMSThread
	{
		...
		public ExternalProcessThread(ArrayList<ArrayList<Object>> rows)
		{
			this.rows = rows;
		}
		...
		private void process(ArrayList<ArrayList<Object>> probe) throws Exception
		{
	...
	private class LeftThread extends HRDBMSThread
	{
		...
		public void run()
		{
			...
			int size = (int)(ResourceManager.QUEUE_SIZE * Double.parseDouble(HRDBMSWorker.getHParms().getProperty("external_factor")) / (numBins >> 2));
			int i = 0;
			while (i < numBins)
			{
