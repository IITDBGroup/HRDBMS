...
public final class InternalConcurrentHashMap implements Serializable
{
	...
	static
	{
		...
		try
		{
			...
			HASHSEED_OFFSET = UNSAFE.objectFieldOffset(InternalConcurrentHashMap.class.getDeclaredField("hashSeed"));
			SEGSHIFT_OFFSET = UNSAFE.objectFieldOffset(InternalConcurrentHashMap.class.getDeclaredField("segmentShift"));
			SEGMASK_OFFSET = UNSAFE.objectFieldOffset(InternalConcurrentHashMap.class.getDeclaredField("segmentMask"));
			SEGMENTS_OFFSET = UNSAFE.objectFieldOffset(InternalConcurrentHashMap.class.getDeclaredField("segments"));
		}
	...
	private static int randomHashSeed(InternalConcurrentHashMap instance)
	{
	 ...
	 */
	public boolean contains(ArrayList<Object> value)
	{
	 ...
	 *             if the specified value is null
	 */
	public boolean containsValue(ArrayList<Object> value)
	{
		// Same idea as size()
		if (value == null)
		{
	...
	private void readObject(java.io.ObjectInputStream s) throws IOException, ClassNotFoundException
	{
		 ...
		 * (;;) { long key = s.readLong(); ArrayList<Object> value =
		 * (ArrayList<Object>)s.readObject(); if (key == -1) break; put(key,
		 * value); }
		 */
	}
	...
	private void writeObject(java.io.ObjectOutputStream s) throws IOException
	{
		 ...
		 * e = e.next) { s.writeLong(e.key); s.writeObject(e.value); } } }
		 * finally { seg.unlock(); } } s.writeLong(-1); s.writeObject(null);
		 */
	}
	...
	public final class WriteThroughEntry
	{
		...
		WriteThroughEntry(long k, ArrayList<Object> v)
		{
			this.k = k;
			this.v = v;
		}
		...
		public ArrayList<Object> setValue(ArrayList<Object> value)
		{
			...
			final ArrayList<Object> v = this.v = value;
			InternalConcurrentHashMap.this.put(this.k, value);
			return v;
		}
	}
	...
	static final class HashEntry
	{
		final int hash;
		final long key;
		volatile ArrayList<Object> value;
		...
		HashEntry(int hash, long key, ArrayList<Object> value, HashEntry next)
		{
			this.hash = hash;
			this.key = key;
			this.value = value;
	...
	static final class Segment extends ReentrantLock implements Serializable
	{
		...
		private static final long serialVersionUID = 2249069246763182399L;
			...
			for (int i = 0; i < oldCapacity; i++)
			{
				...
				if (e != null)
				{
					...
					{ // Reuse consecutive sequence at same slot
						...
						{
							final ArrayList<Object> v = p.value;
							final int h = p.hash;
		 ...
		 */
		private HashEntry scanAndLockForPut(long key, int hash, ArrayList<Object> value)
		{
			// shrinkLock.readLock().lock();
			HashEntry first = entryForHash(this, hash);
			HashEntry e = first;
		...
		final ArrayList<Object> put(long key, int hash, ArrayList<Object> value, boolean onlyIfAbsent)
		{
			// shrinkLock.readLock().lock();
			HashEntry node = tryLock() ? null : scanAndLockForPut(key, hash, value);
			ArrayList<Object> oldValue;
			try
			{
				final HashEntry[] tab = table;
		 ...
		 */
		final ArrayList<Object> remove(long key, int hash, ArrayList<Object> value)
		{
			// shrinkLock.readLock().lock();
			if (!tryLock())
			{
		...
		final ArrayList<Object> replace(long key, int hash, ArrayList<Object> value)
		{
			// shrinkLock.readLock().lock();
			if (!tryLock())
			{
		...
		final boolean replace(long key, int hash, ArrayList<Object> oldValue, ArrayList<Object> newValue)
		{
			// shrinkLock.readLock().lock();
			if (!tryLock())
			{
	...
	final class Values implements Set
	{
		...
		{
			HRDBMSWorker.logger.error("Unimplemented");
			System.exit(1);
			return false;
		}
